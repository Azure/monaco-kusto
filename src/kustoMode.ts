...
export function setupMode(defaults: LanguageServiceDefaultsImpl): WorkerAccessor {
	...
	disposables.push(client);
		...
		const augmentedSetSchema = (schema: Schema, worker: KustoWorker) => {
		...
		return worker.then(worker => ({
			...worker,
			setSchema: (schema, clusterType) => augmentedSetSchema(schema, worker),
			setSchemaFromShowSchema: (schema, connection, database) => {
				worker.normalizeSchema(schema, connection, database)
	...
	disposables.push(monaco.languages.registerCompletionItemProvider(language, new languageFeatures.CompletionAdapter(workerAccessor)));
