...
if (typeof (document) == "undefined") {
    ...
    (self as any).importScripts('../../language/kusto/bridge.min.js');
    (self as any).importScripts('../../language/kusto/kusto.javascript.client.min.js');
    (self as any).importScripts('../../language/kusto/Kusto.Language.Bridge.min.js');
}
 ...
 */
 class KustoLanguageService implements LanguageService {
    ...
    doRangeFormat(document: ls.TextDocument, range: ls.Range): Promise<ls.TextEdit[]> {
        ...
        return Promise.resolve([ls.TextEdit.replace(newRange, formattedText)]);
    }
    ...
    getCommandsInDocumentV1(document: ls.TextDocument): Promise<{absoluteStart: number, absoluteEnd: number, text: string}[]> {
        ...
        return Promise.resolve(commands.map(({AbsoluteStart, AbsoluteEnd, Text}) => ({absoluteStart:AbsoluteStart, absoluteEnd: AbsoluteEnd, text: Text})));
    }
    ...
    getCommandsInDocumentV2(document: ls.TextDocument): Promise<{absoluteStart: number, absoluteEnd: number, text: string}[]> {
        ...
        return Promise.resolve(commands.map(({Start, End, Text}) => ({absoluteStart:Start, absoluteEnd: End, text: Text})));
    }
    ...
    doHover(document: ls.TextDocument, position: ls.Position): Promise<ls.Hover | undefined> {
        ...
        return Promise.resolve({contents: quickInfo.Text});
    }
