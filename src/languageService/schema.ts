...
export const getCallName = (fn: Function): string  => `${fn.name}(${fn.inputParameters.map(p => `{${p.name}}`).join(',')})`;


export const getExpression = (fn: Function): string => `let ${fn.name} = ${getInputParametersAsCslString(fn.inputParameters)} ${fn.body}`;

export const getInputParametersAsCslString = (inputParameters: InputParameter[]): any  => `(${inputParameters.map(inputParameter => getInputParameterAsCslString(inputParameter)).join(',')})`;
